...
export default function Button( { isLoading, children, varient = "primary", ...props } : Props & React.HTMLProps<HTMLButtonElement> ) {
	...
	const className = `text-xs px-3 py-2 ${ styles[ varient ] } font-medium rounded flex items-center space-x-1 active:translate-y-0.5 ${ props.className } disabled:opacity-50`;
	props = {
		...props,
		className,
	}
